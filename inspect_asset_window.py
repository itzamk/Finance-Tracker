...
from investment_database import get_asset_details, get_transaction_details, add_transaction, modify_transaction, delete_transaction
...
class InspectAssetWindow(tk.Toplevel):
    def __init__(self, master, asset_id, app):
        ...
        self.price_label = ttk.Label(self.details_frame, text=f"Current Price: ${self.asset_details[3]:,.2f}")
        self.price_label.pack()

        self.value_label = ttk.Label(self.details_frame, text=f"Total Value: ${self.asset_details[2]*self.asset_details[3]:,.2f}")
